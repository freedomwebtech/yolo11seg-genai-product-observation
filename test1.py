...
def get_last_saved_image():
    ...
    images = sorted(os.listdir(crop_folder), key=lambda x: os.path.getmtime(os.path.join(crop_folder, x)), reverse=True)
    
    for image in images:
        if image.endswith(".jpg"):
            last_saved_image = os.path.join(crop_folder, image)
